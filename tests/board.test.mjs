...
// carry a certified full-clear path, and plan-blind play finishes most ──
{
  ...
  // 씨드가 무엇을 덮는지 밝혀 둔다.
  //
  // 정정(2026-08-26): 여기 "40개 씨드로 재어 보니 6단계만 25%"라고 적혀
  // 있었는데, 검수에서 재현되지 않는다는 지적을 받고 다시 재 보니 틀렸다.
  // 독립 씨드 20개로 생성기가 저장하는 완전클리어 계획만 세어 보면
  //   4단계 0% · 5단계 5% · 6단계 45% · 9단계 5%
  // 가 8판 표본에서 계획이 하나도 안 나오는 비율이다(6단계 평균 0.8/8).
  // 아래 certified는 저장된 계획이 없을 때 solveFullClear로 한 번 더
  // 찾아보므로 실제 비율은 이보다 낮지만, 그 탐색이 느려서 여기서는
  // 큰 표본으로 다시 재지 못했다.
  //
  // 그래서 이 숫자들은 "이 씨드에서 이렇더라"이지 생성 품질의 안정된
  // 지표가 아니다. 품질을 정말 지키고 싶다면 스테이지별 독립 씨드로 더
  // 큰 표본을 도는 별도 시험을 두어야 한다 - 이 시험의 목적은 어디까지나
  // "고친 곳만 다시 재도록 표본을 고정하는 것"이다.
  //
  // 흔들리는 곳이 6단계(6x7, round 6)라는 방향 자체는 그대로다. 그 구간에
  // 완전 클리어 경로가 드문 것은 결함이 아니라 측정된 성질이고(그래서 그
  // 구간은 rescue 그물에 기댄다), 판이 매끄럽게 끝나는지는 아래
  // smoothFinishes 단언이 따로 지킨다.
  //
