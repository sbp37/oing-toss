...
class OingGame {
  constructor() {
    ...
    this.input = attachStickyRectangleInput({
      ...
      isEnabled: () => this.state.running && !this.state.paused && !this.state.inputLocked,
  ...
  preview(rect) {
    ...
    this.ui.previewSelection(rect, this.model.stats(rect));
  }
  ...
  async commit(rect) {
    ...
    if (stats.sum === 10 && stats.count >= 2) await this.handleSuccess(rect, stats);
  ...
  async handleSuccess(rect, stats) {
    ...
    this.state.combo = this.state.lastSuccessAt && now - this.state.lastSuccessAt <= COMBO_WINDOW_MS
      ? this.state.combo + 1
      : 1;
    ...
    this.ui.showScoreBurst(points, rect, this.model.size, this.state.combo);
    this.updateHUD();
    this.speakForSuccess();

    await this.ui.animateSuccess(rect);
  ...
  async clearRound() {
    ...
    await delay(500);
    this.state.round = Math.min(3, this.state.round + 1);
  ...
  tick() {
    ...
    this.state.timeLeft = Math.max(0, (this.endAt - performance.now()) / 1000);
  ...
  updateHUD() {
    const config = getRoundConfig(this.state.round);
    this.ui.updateHUD({ ...this.state, target: config.target });
  }
