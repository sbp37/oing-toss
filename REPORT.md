...
## 2026-08-07 홈 밀도·아이템 버튼 재정리

- 하단 버튼 배경에서 숫자용 무지개 시럽 타일을 제거했다. 힌트는 크림, 섞기는 민트, 폭탄은 코랄, 시계는 하늘색의 독립 토이 버튼 문법으로 통일하고 도톰한 하단 깊이와 짧은 눌림을 CSS로 구현했다.
- 네 아이템 기능과 독립 아이콘·HTML 이름·횟수 배지는 유지했다. 폭탄 아이콘 원본 크기도 286×312로 바로잡고 네 아이콘을 플레이 진입 전에 미리 불러온다.
- 늘어난 플레이 하단 행을 가용 공간의 중앙에 배치해 4×4 보드와 아이템 사이의 큰 공백을 줄였다. 고양이와 말풍선은 버튼 위에서 겹치지 않는 고정 스테이지를 유지한다.
- 홈은 손 흔들기 대신 승인된 기본 앉기 포즈를 크게 배치하고, 고양이 아래의 부드러운 잔디빛 받침·작은 꽃·조작 설명을 추가했다. 설정은 좌상단 단독 아이콘에서 배경 하단의 아이콘+라벨로 이동했다.
- 실제 브라우저에서 360×780, 390×844, 430×932를 확인했다. 각 크기에서 문서 크기가 뷰포트와 같고 버튼·보드 잘림과 가로/세로 스크롤이 없었다. 아이템 버튼은 화면에 따라 약 78–85×65–74px로 표시된다.
- 이번 변경은 `index.html`과 `css/styles.css`의 에셋 배치·스타일만 다뤘으며 `js/input.js`와 게임 규칙은 수정하지 않았다.

## 2026-08-06 초반 플레이·힌트·아이템 슬롯 다듬기

- 새 드래그가 시작될 때 이전 시럽 선택막의 좌표가 1프레임 노출되지 않도록, 숨은 선택막을 새 좌표에 먼저 배치한 뒤 표시한다. 화면 갱신 전에 도착하는 첫 선택 스냅도 숨은 상태에서는 실행하지 않는다.
- 일반 합10 성공에서 보드 전체를 흔들던 `success-kick`을 제거했다. 선택 타일의 눌림·팝·파편만 유지해 판 자체는 안정적으로 보인다.
- 첫 5회 성공까지는 기본 난이도보다 합10 후보를 최소 3개, 인접한 두 칸짜리 답을 최소 2개 더 확보한다. 첫 조작 안내와 힌트는 이 가운데 중앙에 가까운 두 칸 답을 우선 선택한다.
- 첫 조작 안내는 답 전체를 감싸는 코랄·화이트 이중 포커스와 반짝이로 강화하고 문구를 `두 칸을 슥 밀어서 10!`으로 바꿨다.
- 힌트 버튼에서 답으로 날아가던 노란 연결선은 제거했다. 답 영역 자체에 민트·화이트 포커스, 코랄 라벨, 모서리 반짝이만 1.45초 표시한다.
- 섞기는 기존 타일 산개·재집결 사이에 중앙 회전 아이콘, 번개 3개와 바깥으로 퍼지는 반짝이 4개를 함께 표시한다.
- 하단 아이템은 긴 가로 버튼 2개에서 힌트·섞기·폭탄·시계 4개의 정사각 시럽 슬롯으로 변경했다. 독립 아이콘, HTML 이름과 원형 횟수 배지는 그대로 유지한다.
- 정규·아이템·점수 테스트 16건과 일반/섞기 750개, 초반 보조 300개 보드 생성을 통과했다. `js/input.js`는 수정하지 않았다.
- 인앱 브라우저가 로컬 `file://` 새로고침을 차단해 이번 변경분의 실제 브라우저 캡처는 만들지 못했다. 정적 빌드 및 수치 검수 후 실제 모바일에서 최종 시각 확인이 필요하다.

## 2026-08-06 메가폭탄 드롭

- 콤보 14부터 보드 드롭 풀에 메가폭탄을 추가했다. 일반 폭탄·시계와 마찬가지로 빈칸에 등장하고 같은 칸을 한 번 탭해야만 발동한다.
- 원조 OING의 보상 수치인 최대 12개 숫자 제거, 제거 숫자 합계 + 40점, 콤보 +1을 유지했다. 범위는 원조의 편향된 순회 대신 아이템 중심 5×5 안에서 가까운 숫자부터 대칭적으로 선택한다.
- 전용 효과음은 원조 `playMegaBomb()`의 긴 저역 충격음과 800/1200/600/400/1600Hz 파편 5겹 구조를 재사용했다.
- 전용 아이콘은 기존 일반 폭탄을 스타일 기준으로 이미지 생성한 뒤 크로마키 제거했다. 원본 투명 PNG는 1254×1254, 배포본은 512×512 lossless WebP다.
- 일반 폭탄보다 큰 이중 링, 5방향 파편, 최대 12개 타일 팝, 강한 보드 킥과 전용 점수 팝업을 사용하되 전체 화면 플래시는 넣지 않았다.
- 자동 테스트는 16건을 통과했고 `js/input.js`는 수정하지 않았다.

## 2026-08-06 원조형 보드 아이템 드롭 1차

- 콤보가 7의 배수를 처음 넘을 때 폭탄 또는 시계를 보상 대기열에 넣고, 합10 제거로 생긴 빈칸에 독립 아이템 셀로 등장시킨다.
- 보드 아이템은 숫자 값이 없는 셀이므로 합계·정답 탐색·직사각형 드래그 결과에 영향을 주지 않는다. 숫자 선택 중에도 아이템 셀 자체는 선택색이나 성공/실패 타일 효과를 받지 않는다.
- 아이템 위에서 시작한 포인터는 기존 드래그 입력으로 전달하지 않는다. 이동 거리가 10px를 넘으면 사용을 취소하고, 같은 아이템 칸에서 손을 놓은 한 번의 탭만 발동한다.
- 폭탄은 등장 칸 중심의 최대 3×3 숫자를 즉시 제거하고 `합계 + 20` 점수를 지급한다. 시계는 즉시 8초를 더하고 아이콘이 시간 HUD로 날아간다.
- 사용하지 않은 보드 아이템은 라운드/정답 보장용 새 보드 전환 때 대기열로 이동하며, 다음 빈칸이 생기면 다시 나타난다.
- `freeze`, `clover`, `candy`는 원조 ID와 단계별 드롭 풀만 예약했다. 아직 실제 풀에는 들어가지 않으며 잘못 눌리는 미완성 아이템도 화면에 나오지 않는다.
- 새 `BoardItemField` 단위 테스트를 포함해 자동화 15건이 통과했다. `js/input.js`와 승인된 드래그 함수는 수정하지 않았다.
...
| 뷰포트 | 계산된 보드 외곽 | 아이템 슬롯 1개 예상 크기 |
|---|---:|---:|
| 280×653 | 256px | 약 60×53px |
| 360×780 | 332px | 약 78×65px |
| 390×844 | 362px | 약 85×74px |
| 430×932 | 402px | 약 84×74px |
...
`bomb`, `clock`, `megabomb`은 현재 실제 플레이 아이템으로 연결했다. `freeze`와 `clover`는 에셋 경로와 `useFutureItem` 훅만 유지하며 화면에는 노출하지 않는다.
